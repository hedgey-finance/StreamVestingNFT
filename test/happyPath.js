...
module.exports = (vesting, bound, amountParams, timeParams) => {
  let streaming, creator, a, b, c, token, usdc, batchMinting;
  let amount, now, start, cliff, rate, end, admin, unlock;
  it('Mints an NFT to wallet A', async () => {
    let s;
    if (vesting == true) {
  ...
  it('Wallet A redeems the NFT', async () => {
    ...
    if (remainder > 0) {
      ...
      newTime = await time.latest();
      await time.increase(end - newTime);
